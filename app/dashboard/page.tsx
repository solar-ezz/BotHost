  ...
  return (
    <div className="p-8 max-w-6xl">
      <div className="mb-8">
        ...
        <p className="text-surface-500 dark:text-surface-400 mt-1 text-sm">Manage and monitor your Discord bots</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-white dark:bg-surface-900 border border-surface-100 dark:border-surface-800 rounded-xl p-5">
          <p className="text-xs font-medium text-surface-400 uppercase tracking-wide mb-1">Total Bots</p>
          <p className="text-3xl font-bold text-surface-900 dark:text-white">{bots.length}</p>
        </div>
        <div className="bg-white dark:bg-surface-900 border border-surface-100 dark:border-surface-800 rounded-xl p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-semibold text-surface-900 dark:text-white">Your Bots</h2>
        <Link
          href="/bots/new"
      ...
      {bots.length === 0 ? (
        <div className="bg-white dark:bg-surface-900 border border-surface-100 dark:border-surface-800 rounded-xl p-16 text-center">
          <div className="w-12 h-12 bg-surface-100 dark:bg-surface-800 rounded-xl flex items-center justify-center mx-auto mb-4">
            <Bot className="w-6 h-6 text-surface-400" />
          </div>
          <h3 className="font-semibold text-surface-900 dark:text-white mb-2">No bots yet</h3>
          <p className="text-sm text-surface-500 dark:text-surface-400 mb-6">Add your first Discord bot to get started</p>
          <Link
            href="/bots/new"
            className="inline-flex items-center gap-2 bg-brand-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-700 transition-colors"
          >
          ...
          {bots.map(bot => {
            ...
            return (
              <Link
                key={bot.id}
                href={`/bots/${bot.id}`}
                className="bg-white dark:bg-surface-900 border border-surface-100 dark:border-surface-800 rounded-xl p-5 hover:border-brand-200 dark:hover:border-brand-700 hover:shadow-sm transition-all flex items-center gap-4"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1">
                    <span className="font-semibold text-surface-900 dark:text-white">{bot.name}</span>
                ...
                {latestMetric && bot.status === 'RUNNING' && (
                  <div className="hidden sm:flex items-center gap-4 text-xs text-surface-500 dark:text-surface-400">
                ...
                <ExternalLink className="w-4 h-4 text-surface-300 dark:text-surface-600 shrink-0" />
              </Link>
            )
          })}
        </div>
